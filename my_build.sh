...
if [ $MODEL = noblelte ]
then
	# KERNEL_DEFCONFIG=lineageos_nobleltexx_defconfig
	KERNEL_DEFCONFIG=ultimate_defconfig
...
then
	...
	KERNEL_DEFCONFIG=lineageos_zenltexx_defconfig
fi
fi
fi
fi
...
(
	...
	FUNC_BUILD_KERNEL
	# FUNC_BUILD_RAMDISK # not run this one
	mv $RDIR/arch/$ARCH/boot/Image $RDIR/arch/$ARCH/boot/boot.img-zImage
	mv $RDIR/arch/$ARCH/boot/dtb.img $RDIR/arch/$ARCH/boot/boot.img-dtb
